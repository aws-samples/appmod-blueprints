...
Merges global ackPodIdentity with per-addon ackPodIdentity config.
Supports both path-based sourcing (git repo) and repoURL/chart-based sourcing (Helm repo).
Uses enableAckPodIdentity as the key name for enabling pod identity.
*/}}
...
- repoURL: '{{ $values.repoURLGit }}'
  ...
  path: {{ default "charts/pod-identity" .path }}
{{- else if .repoURL }}
- repoURL: '{{ .repoURL }}'
  chart: '{{ .chart }}'
