...
resource "aws_secretsmanager_secret_version" "cluster_config" {
  ...
  secret_string = jsonencode({
    metadata = jsonencode({
      ...
      aws_cluster_arn               = aws_eks_cluster.hub.arn
      aws_vpc_id                    = aws_vpc.hub.id
      aws_subnet_ids                = join(",", concat(aws_subnet.public[*].id, aws_subnet.private[*].id))
      aws_private_subnet_ids        = join(",", aws_subnet.private[*].id)
      aws_cluster_security_group_id = aws_eks_cluster.hub.vpc_config[0].cluster_security_group_id
    ...
    vpc = jsonencode({
      ...
      cluster_security_group_id = aws_eks_cluster.hub.vpc_config[0].cluster_security_group_id
    })
  })
}
