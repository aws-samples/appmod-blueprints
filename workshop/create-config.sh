...
# CloudFront infra (internal ALB + VPC origin + distribution) is provisioned by the
# shipped, reusable scripts/provision-cloudfront-infra.sh — the SAME script an external
# consumer runs — so this logic lives in exactly one place instead of being duplicated here.
#
# Launched in the BACKGROUND: the distribution takes 5-15 min, while `task install` spends
# ~20 min building the cluster before it needs the hostname. domain:resolve then runs
# scripts/resolve-cloudfront-domain.sh, which finds this distribution by its Comment, so
# the hostname never has to be handed between the two steps.
_INFRA_LOG="/tmp/create-config-infra.log"
if [ -n "${HUB_VPC_ID:-}" ]; then
  echo "  ▸ Provisioning CloudFront infra in background (log: $_INFRA_LOG)"
  HUB_CLUSTER_NAME="${RESOURCE_PREFIX:-peeks}-hub" \
  VPC_ID="$HUB_VPC_ID" \
  SUBNET_IDS="${HUB_SUBNET_IDS:-}" \
  AWS_REGION="$REGION" \
    nohup "$REPO_ROOT/scripts/provision-cloudfront-infra.sh" >> "$_INFRA_LOG" 2>&1 &
  echo "  ▸ Background PID=$! — a failure here surfaces as a domain:resolve timeout"
fi
