...
- ALWAYS validate YAML syntax after modifying any YAML file: use `yq eval '.' <file> > /dev/null` for general YAML (values.yaml, config files), or `kubectl apply --dry-run=client -f <file>` for Kubernetes manifests (ID: TROUBLESHOOT_VALIDATE_YAML)
